...
- [x] **`admin` 初始密碼的處理方式（2026-08-13 決策：維持現狀）**：Liquibase `001-initial-schema.xml` 會建立 admin 並帶一組寫在 changelog 裡的公開初始密碼。評估後決定**不改**——這是初始密碼的常見做法（Jenkins、Grafana、ArgoCD 都是這個模式），由安裝者在首次登入後自行更改
  - 配套已補上：`README.md` 的快速啟動段落與 `system-architecture.md` 的種子帳號段落都加了「服務對外開放前先改掉 admin 密碼」的提醒，讓這個約定是明寫的而不是隱含的
  - 若未來想更嚴謹，業界更強的做法是「每次安裝隨機產生」或「強制首次登入改密碼」，而不是固定值；需要時再評估
