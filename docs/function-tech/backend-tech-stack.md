...
### 核心架構亮點：
1.  **多維度篩選**: 擴展數據模型以支援 `product_id` (產品)、`lot_id` (批次) 與 `wafer_id` (晶圓) 的階層式篩選，符合半導體 MES 系統標準。
2.  **非同步 I/O**: 充分利用 FastAPI 的 `async` 特性，在高併發請求下仍能保持低延遲。
3.  **分頁處理**: 實作 `iloc` 切片邏輯，確保海量工業數據在回傳給前端時是有序且可控的。
4.  **異質整合**: 後端可同時處理來自 Parquet 格式的科學數據與來自 PostgreSQL 的權限數據。
